...
function Issues({ formik, caseDetails, disableForm, isReEdit }) {
  ...
  useEffect(() => {
    if (disableForm || isReEdit) {
      [...values.otherIssues].forEach((issue) => {
        if (issue.issueType) fetchSubIssuesList(issue.issueType);
      });
    }
  ...
  const fetchSubIssuesList = async (issueTypeId) => {
    ...
    if (!subIssues[issueTypeId]) {
      try {
        ...
        const response = await client.post(nmiListURL, body);
        setSubIssues((prevState) => ({
          ...prevState,
          [issueTypeId]: response,
        }));
      } catch (errorResponse) {
        const newErrMsgs = getMsgsFromErrorCode(
          `POST:${process.env.REACT_APP_NMI_LIST}`,
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  disabled={
                    disableForm || values.otherIssues[index].editable === false
                  }
                />
              }
              sx={{
              ...
              <Stack>
                <FormControl sx={{ width: "13rem" }} size="small">
                  ...
                  <Select
                    ...
                    disabled={
                      !values.otherIssues?.[index].selected ||
                      disableForm ||
                      values.otherIssues[index].editable === false
                    }
                  >
                    {issues.map((issue) => (
